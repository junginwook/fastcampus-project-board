...
class ArticleControllerTest {
	...
	@DisplayName("[view][GET] 게시글 리스트 (게시판) 상세 페이지 - 페이징, 정렬 기능")
	@Test
	...
	void givenHashtag_whenRequestingArticleSearchHashtagView_thenReturnsArticleSearchHashtagView() throws Exception {
		...
		mvc.perform(
				...
				.andExpect(model().attribute("articles", Page.empty()))
				.andExpect(model().attribute("hashtags", hashtags))
				.andExpect(model().attribute("paginationBarNumbers", List.of(1,2,3,4,5)))
				.andExpect(model().attribute("searchTypes", SearchType.HASHTAG));
	...
	@WithUserDetails(value = "inwookTest", setupBefore = TestExecutionEvent.TEST_EXECUTION)
	...
	void givenUpdatedArticleInfo_whenRequesting_thenUpdatesNewArticle() throws Exception {
		//Given
		long articleId = 1L;
		ArticleRequest articleRequest = ArticleRequest.of("test title", "test content");
